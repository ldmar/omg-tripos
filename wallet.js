/* ============================================================
   ...
   Storage de archivos · IndexedDB + Blobs
   ============================================================ */
...
export async function getFilesByEvent(eventId) {
  if (!eventId) return [];
  const idx = await db().then(idb => new Promise((resolve, reject) => {
    const t = idb.transaction(STORE_FILES, 'readonly');
    const index = t.objectStore(STORE_FILES).index('eventId');
