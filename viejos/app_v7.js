...
async function deleteEvent(id) {
  // Borrar también los archivos adjuntos
  try {
...
let editingAttachments = [];   // [{ id, isNew, file?, record? }]
...
async function openModal(id) {
  ...
  if (isEdit) {
    try {
      const files = await wallet.getFilesByEvent(e.id);
      editingAttachments = files.map(f => ({ id: f.id, isNew: false, record: f }));
    } catch {}
  }
  renderAttachments();
...
function renderAttachments() {
  ...
  list.querySelectorAll('[data-remove]').forEach(btn => {
    btn.addEventListener('click', (ev) => {
      ...
      // Si es existente, borrarlo del storage
...
form.addEventListener('submit', async ev => {
  ...
  for (const a of editingAttachments) {
    if (a.isNew && a.file) {
      try {
        await wallet.saveFile(a.file, { eventId: id, eventType: currentType });
      } catch (err) {
...
/* ============================================================
   Refrescar subsistemas cuando cambian los eventos
   ============================================================ */
