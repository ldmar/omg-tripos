/* ============================================================
   ...
   IndexedDB + render + CRUD + PWA + Import Mágico + Sync P2P
   ============================================================ */
...
/* ============================================================
   IMPORT · Drag & drop unificado (PDF + imagen)
   ============================================================ */
...
function updateSyncStatus(mode = 'auto') {
  ...
  if (!online) {
    shareStatus.classList.add('is-offline');
    shareStatusText.textContent = 'Sin internet · el link seguirá funcionando cuando vuelvas';
    return;
  }
  if (connected) {
...
/* ---------- Join: viene con ?join=ROOM en la URL ---------- */
(function handleJoin() {
  const params = new URLSearchParams(location.search);
  ...
  if (m && m[1]) {
    localStorage.setItem('ohmygoch_sync_secret', m[1]);
  }
  history.replaceState({}, '', location.pathname);
})();
