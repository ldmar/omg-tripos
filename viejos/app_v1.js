/* ============================================================
   ...
   IndexedDB + render + CRUD + PWA hooks + Import Mágico
   ============================================================ */

import { analyze, PROVIDERS, EXAMPLES } from './parser.js';
...
analyzeBtn.addEventListener('click', () => {
  ...
  const result = analyze(text, importProvider);
  importResult = result;
...
document.getElementById('importConfirmBtn').addEventListener('click', async () => {
  ...
  const selected = importResult.events.filter((_, i) => importSelected.has(i));
  ...
  closeImport();
  setView('today');
  toast(`✓ ${selected.length} evento${selected.length > 1 ? 's' : ''} agregado${selected.length > 1 ? 's' : ''}`);
});
...
function renderImportPreview(result) {
  ...
  summary.textContent = result.events.length === 1
    ? 'Se detectó 1 evento'
    : `Se detectaron ${result.events.length} eventos`;
  ...
  if (!result.events.length) {
    ...
    eventsEl.innerHTML = result.events.map((ev, i) => {
      ...
      return `
        <div class="import-event is-selected" data-idx="${i}">
          <div class="import-event__icon">${meta.icon}</div>
          <div class="import-event__body">
            <div class="import-event__type">${meta.label}</div>
            <div class="import-event__title">${escapeHtml(ev.title)}</div>
