...
export function analyze(text, forced = 'auto') {
  ...
  // Detección automática: probar cada provider en orden de especificidad
  const order = ['booking', 'airbnb', 'despegar', 'flight'];
  ...
  return results[0]?.events.length
    ? results[0]
    : { provider: null, providerName: null, events: [], warnings: ['No pude identificar el proveedor ni extraer eventos.'] };
}
