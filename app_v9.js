...
async function loadAll() {
  ...
  if (!trip) {
    ...
    trip = { id: 'default', title: 'Mi viaje', flag: '🌍', days: 0, travelers: 1 };
  }
...
function renderTrip() {
  ...
  if (flagEmoji) flagEmoji.textContent = trip.flag || '🌍';
  if (titleEl) titleEl.textContent = trip.title || 'Mi viaje';
  if (heroName) heroName.textContent = trip.title || 'Mi viaje';
...
function renderTimeline() {
  ...
  if (!events.length) {
    ol.innerHTML = '';
    empty.hidden = false;
    if (emptyTitle) emptyTitle.textContent = `Sin eventos en "${trip.title}"`;
...
async function startSync() {
  try {
    ...
    // Publicar meta del viaje para que los peers la reciban
    sync.setTripMeta({
      title: trip.title,
      flag: trip.flag,
    ...
    // Cambios en eventos desde otros peers
    ...
    // Cambios en la meta del viaje desde otros peers
    sync.onTripMetaChange(async (meta) => {
      ...
      const updated = await trips.updateTrip(trip.id, {
        title: meta.title,
        flag: meta.flag,
    ...
    // Link de compartir con tripId VISIBLE + secret
    ...
    qrLoaded = false;
    updateSyncStatus();
    console.log('🔗 Sync activo · room:', roomId, '· viaje:', trip.title);
  } catch (err) {
...
async function handleJoin() {
  ...
  // Formato nuevo: tripId~secret
  const parts = join.split('~');
  if (parts.length !== 2) {
    console.warn('🔗 Link de join con formato desconocido:', join);
    return false;
  }

  const [joinTripId, secret] = parts;
  if (!joinTripId || !secret) return false;

  // Guardar el secret por viaje
  localStorage.setItem(`ohmygoch_secret_${joinTripId}`, secret);
  console.log('🔗 Join detectado · viaje:', joinTripId, '· secret:', secret);

  // ¿Existe el viaje localmente?
  const existing = await trips.getTrip(joinTripId);

  if (!existing) {
    // Crear viaje placeholder — se va a actualizar vía sync al conectarse
    ...
    console.log('✨ Viaje compartido creado (placeholder)');
  }
    ...
    return `
      <div class="trip-item ${isActive ? 'is-active' : ''}" data-trip-id="${t.id}" role="button" tabindex="0">
        ...
        <div class="trip-item__body">
          <p class="trip-item__title">${escapeHtml(t.title)}</p>
...
function openTripCreateModal() {
  ...
  // Mostrar opción de clonar solo si el viaje actual tiene eventos
...
tripRenameForm.addEventListener('submit', async (ev) => {
  ...
  // Si es el activo, actualizar en memoria
  if (trip?.id === id) {
    trip = await trips.getActiveTrip();
    renderTrip();
  }
...
(async function boot() {
  // 1. IMPORTANTE: manejar join ANTES de inicializar todo
  await handleJoin();

  // 2. Inicializar multi-viaje (con migración si hace falta)
  ...
  if (IS_DEV) {
    ...
    console.log('📚 Viaje activo:', trip?.title, '·', events.length, 'eventos');
  } else {
    registerSW();
  }
})();
